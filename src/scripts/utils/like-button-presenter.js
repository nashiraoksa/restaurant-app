...
const LikeButtonPresenter = {
  ...
  async _renderButton() {
    const { id } = this._resto.restaurant;
  ...
  _renderLike() {
    ...
    likeButton.addEventListener('click', async () => {
      await this._favoriteResto.putResto(this._resto.restaurant);
      this._renderButton();
    });
  },
  ...
  _renderLiked() {
    ...
    const likeButton = document.querySelector('#likeButton');
    likeButton.addEventListener('click', async () => {
      await this._favoriteResto.deleteResto(this._resto.restaurant.id);
      this._renderButton();
    });
  },
};
